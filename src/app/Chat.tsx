...
export default function Chat() {
  ...
  return (
    <div className='w-full max-w-md py-4 flex flex-col'>
      <div>
        <span className='text-xl font-bold'>Chat</span>
      </div>

      <div className='flex-1 min-h-[32rem] mb-2 p-4 bg-neutral-950 border border-neutral-700 rounded'>
        {messages.map(m => (
          <div key={m.id}>
            {m.role === 'user' ? 'User: ' : 'AI: '}
            {m.content}
      ...
      <form onSubmit={handleSubmit} className='flex flex-row justify-between gap-2'>
        <input
          className='flex-1 max-w-md p-2 bg-neutral-900 border border-neutral-700 rounded shadow-xl'
          value={input}
